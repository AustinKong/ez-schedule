...
export async function getGroupByGroupId(groupId) {
	const db = await connectDb();
	try {
...
export async function getGroupsManagedByManagerId(managerId) {
	...
	try {
		...
		const groups = await db
			.collection('groups')
			.find({
				_id: { $in: manager.managedGroups.map((id) => new ObjectId.createFromHexString(id)) },
			})
			.toArray();
...
export async function getGroupsEnrolledByUserId(userId) {
	...
	try {
		...
		const user = await db
			.collection('users')
			.findOne({ _id: new ObjectId.createFromHexString(userId), userRole: 'user' });
		if (!user) {
...
export async function addMultipleMemberUsersToGroup(groupId, users) {
	...
	try {
		...
		const userIdsToAdd = users.map((user) => new ObjectId.createFromHexString(user.userId));
