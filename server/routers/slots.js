...
async function loadSlot(req, res, next) {
	...
	if (!slot || slot.isClosed) return res.status(404).json({ error: 'Slot not found or closed' });
	slot.entries = await Promise.all(slot.entries.map((entryId) => getEntryById(entryId)));
	req.slot = slot;
	next();
}
...
router.post('/:slotId/leave', loadSlot, async (req, res) => {
	...
	const matchingEntryId = req.slot.entries.find((entry) => entry.visitor === userId)?._id;
	if (!matchingEntryId) return res.status(400).json({ error: 'You are not in the queue' });
...
router.get('/:slotId/position', loadSlot, (req, res) => {
	...
	const index = req.slot.entries.findIndex((entry) => entry.visitor?.toString() === userId);
...
router.put('/:slotId/timing', loadSlot, async (req, res) => {
    ...
    await updateSlotTiming(req.slot._id.toString(), new Date(start), new Date(end));
    const updatedSlot = await getSlotById(req.slot._id);
