...
router.post('/createGroup', async (req, res) => {
	try {
		const { name, description, maxUsers, memberParticipants, password } = req.body;
		const userId = req.user.userId;
...
router.patch('/:groupId/editGroup', async (req, res) => {
	...
	try {
		...
		if (group.createdBy !== req.user.userId) {
			return res.status(403).json({ error: 'Only host can update group details.' });
		}
	...
	try {
		...
		if (result.modifiedCount === 0) {
			return res.status(400).json({
				message: 'No new participants were added (they may already be members).',
			});
		}
