...
const validationSchema = Yup.object().shape({
  ...
  endTime: Yup.date()
    ...
    .test(
      ...
      value => value - this.parent.startTime >= 15 * 60 * 1000
    ),
  location: Yup.string()
  ...
  const handleSubmit = async (values) => {
    try {
      const processed = {
        ...
        start: new Date(values.startTime).toISOString(),
        end: new Date(values.endTime).toISOString()
      };
      
      await onSubmit(processed);
    } catch (error) {
      toaster.error('Failed to save timeslot');
    }
  };
  ...
  return (
    <Formik
      initialValues={timeslot ? {
        ...
        startTime: formatDateTimeLocal(timeslot.start),
        endTime: formatDateTimeLocal(timeslot.end)
      } : {
        name: '',
        startTime: '',
        endTime: '',
    ...
    >
      {({ values }) => (
        <Form className="space-y-4">
          ...
          >
            {isLoading ? 'Saving...' : timeslot?._id ? 'Update Consultation Slot' : 'Create Consultation Slot'}
          </button>
        </Form>
      )}
    </Formik>
